...
export function JacketRail({ id, position, rotation, assignedProducts, onRemove, onOpenSelector, isEditable }: JacketRailProps) {
  ...
  return (
    <group position={position} rotation={rotation}>
      ...
      {/* 2 Rails with 8 slots each (Total 16) */}
      {[0, 1].map((r) => {
                ...
                <group key={slotIndex} position={[offsetX, 0, 0]}>
                  ...
                  {isEditable && (
                    <Html position={[0, -0.85, 0.2]} center>
                      <button 
                        onClick={(e) => {
                          e.stopPropagation();
                          onOpenSelector?.(id, slotIndex, 'jacket');
                        }}
                        style={{ 
                          fontSize: '0.55rem', 
                          padding: '2px 6px', 
                  ...
                  {assigned ? (
                    <group position={[0, -0.2, 0.1]}>
                      ...
                      <Text position={[0, -0.75, 0.3]} fontSize={0.05} color="#fff" anchorX="center" maxWidth={0.4}>
                        {assigned.name}
                      </Text>
                      <Text position={[0, -0.85, 0.3]} fontSize={0.04} color="#c8ff1d" anchorX="center">
                        {`€${assigned.list_price}`}
                      </Text>
                    </group>
                  ) : (
